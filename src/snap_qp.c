...
static int devx_cq_init(struct snap_cq *cq, struct ibv_context *ctx, const struct snap_cq_attr *attr)
{
	...
	if (!attr->cq_on_dpa) {
		...
		if (attr->dpa_element_type == MLX5_APU_ELEMENT_TYPE_THREAD) {
			...
			if (attr->use_eqn || attr->dpa_element_type == MLX5_APU_ELEMENT_TYPE_EMULATED_DEV_EQ)
				devx_cq->eqn_or_dpa_element = attr->eqn;
			else
		...
		cq_mem_size = attr->cqe_size * devx_cq->cqe_cnt;
	...
	DEVX_SET(cqc, cqctx, dbr_umem_id, dbr_umem_id);
	DEVX_SET64(cqc, cqctx, dbr_addr, dbr_addr);
